...
// 接續 week12-1圖+cos() sin() week11-1程式
...
void draw() {
  ...
  if (frameCount%60==0) {//每60frames(1秒)冒出一隻地鼠
    ...
    a[i][j] = 60;//會亂數1格, 站起來60frames(1秒)
  }
  for (int i=0; i<3; i++) {//左手i
    for (int j=0; j<3; j++) {//右手j
      float x = j*100, y = i*100+100;//推算座標, 小心y往下100(藏白色版)
      //if (i==0 && j==0) y += mounseY;//暫時實驗用,看到畫面壞了
      if (a[i][j]>0) {//如果有地鼠
        a[i][j] -= 1;//生命的倒數計時
        y -= 100*sin(a[i][j]*PI/60);//小心y對應 sin()
